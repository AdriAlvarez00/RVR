...
int main(int argc, char** argv)
{
	...
	int rname = getnameinfo(&client,clientLen,host,NI_MAXHOST,service,NI_MAXSERV,NI_NUMERICHOST | NI_NUMERICSERV);
    if(rname!=0){
	...
	char buffer[80];
	memset(buffer,0,80);
    	int bytesrec = recv(client_sd,(void*)buffer,79,0);
	if(bytesrec <= 0){
	...
	int bytessent = send(client_sd,buffer,bytesrec,0);

	if(bytessent != bytesrec){
